...
s=$(echo -n $(($(date '+%s')-$(date '+%s')%$(($((60*60))/4))))r3x1m_ataf \
| openssl sha1); DATE=$(date '+%a %d/%m  %R')

ATAF="http://www.temporealeataf.it/Mixer/Rest/PublicTransportService.svc\
/schedule?nodeID=$1&lat=43.8&lon=11.2&s=$s"

BIN=${0:A}; DIR=${BIN%/*}; NAME=${${BIN##/*/}%.*}; LCD=$NAME.lcd; JSON=$NAME.json

cd $DIR/../txt; curl $ATAF > $JSON; MIN=$(date -u '+%R' | awk -F: '{print (($1*60)+$2)}')
